    ...
    text = text.replace(/^(\#{1,6})[ \t]*(.+?)[ \t]*\#*[ \t]*(\((.*)\))?\n+/gm,
        function(wholeMatch,m1,m2,m3,m4) {
            ...
            if (m1 == "#") {
                ...
                cls = m4 ? m4 : h2_options[m2];
            }
            if (cls && cls.length > 0) {
                cls += " header_icon";
            }
            if (h_level == 1) {
                otherHtml = '<a href="javascript:void(0);" class="source">View source ...</a>';
            }
            return hashBlock("<h" + h_level + " class='"+ cls +"'>" + _RunSpanGamut(m2) + otherHtml + "</h" + h_level + ">");
        });
...
var _DoCodeGL = function (text) {

    text = text.replace(/\^([^\s]*)\^/g, function (wholeMatch, m1) {
        var val = m1.toUpperCase();
        return '<code class="gl '+ m1 +'">' + val + '</code>'
    });
