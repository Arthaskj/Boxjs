# Box.ClassManager (singleton)

---

Box.ClassManager 是一个管理类对象的单例。 她提供了一系列方法让开发者可以方便的构建出一套完善的基于模块化开发的应用。为什么需要使用模块化来开发前端，请浏览 [关于Box Library](#/abort)。一般不直接访问它，而是通过以下这些方法（方便记忆）：
